...
public login = async (token: string, role: string): Promise<UserEntity | CompanyEntity | AdminEntity | string> => {
  const result = await this.authRepository.login(token, role);
  return result;
}
