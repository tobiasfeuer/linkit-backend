...
export class AdminUseCase {
  ...
  public deleteAdmin = async (id: string, reqID?: string, total?: string): Promise<AdminEntity[]> => {
    const deleteAdmin = await this.adminRepository.deleteAdmin(id, reqID, total)
    return deleteAdmin
  }

  public deleteAdminPermanently = async (id: string): Promise<AdminEntity[]> => {
    const deletePermanently =await this.adminRepository.deleteAdminPermanently(id);
