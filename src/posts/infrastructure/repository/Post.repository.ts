...
import { PostTranslation } from '../helpers/PostTranslate'
...
export class MongoPostRepository implements PostRepository {
  ...
  async findPost (value: string, filter: string, lang?: string): Promise<PostEntity | PostEntity[]> {
    try {
      let result
      const validFilters = ['title', 'type', 'archived', 'category']
      if (filter === 'all') {
        result = await Post.find()
        if (lang === 'en') result = await PostTranslation(result)
      } else if (filter === 'id') {
  ...
  async findOneBySlug(slug: string, type: string): Promise<PostEntity | null> {
    try {
