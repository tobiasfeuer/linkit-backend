import { type JdEntity, MongoJd } from '../../domain/jd/jd.entity'
...
export class MongoJdRepository implements JdRepository {
    ...
    try {
      ...
      if (!combined) {
        if (filter === 'all') result = await Jd.find()
          ...
          // Siempre devolver la estructura completa con ambos idiomas
          const jdObject = jd.toObject()
          const jdForFrontend = {
            ...jdObject,
            jobType: jdObject.type, // Mapear type a jobType para el frontend
            createdAt: jdObject.createdDate // Mapear createdDate a createdAt
          }
          return jdForFrontend as JdEntity
  ...
  async getJDByCode(code: string): Promise<MongoJd | null> {
  const jd = await Jd.findOne({ code })
  return jd ? (jd.toObject() as MongoJd) : null
}

async updateJD(id: string, jd: JdEntity): Promise<JdEntity> {
  try {
    await validateJD(jd)
    jd.stack = jd.stack?.map(stack => stack.toLowerCase())
    const updated = await Jd.findByIdAndUpdate(id, jd, { new: true })
    if (!updated) throw new ServerError('JD not found', 'Vacante no encontrada', 404)
    return updated.toObject() as JdEntity
  } catch (error: any) {
    if (error instanceof ServerError) throw error
    else throw new UncatchedError(error.message, 'updating JD', 'actualizar vacante')
  }
}
